  ...
  const migrateData = async () => {
    ...
    try {
      ...
      if (journalEntries.length > 0) {
        ...
        if (error)
          throw new Error(`Error migrating journal entries: ${error.message}`);
      ...
      if (exercises.length > 0) {
        ...
        if (error)
          throw new Error(`Error migrating exercises: ${error.message}`);
      ...
      if (courses.length > 0) {
        ...
        if (coursesError)
          throw new Error(`Error migrating courses: ${coursesError.message}`);
        ...
        if (grades.length > 0) {
          const { error: gradesError } = await supabase.from("grades").insert(
            grades
              .map((grade) => {
                const newCourseId = grade.courseId
                  ? courseIdMap.get(grade.courseId)
                  : null;
          ...
          if (gradesError)
            throw new Error(`Error migrating grades: ${gradesError.message}`);
      ...
      if (settings.length > 0) {
        ...
        if (error)
          throw new Error(`Error migrating settings: ${error.message}`);
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>
          Migrate your data from browser storage to secure cloud storage
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {status === "idle" && (
          <div className="space-y-4">
            <p>
              Your data is currently stored in your browser. This means it's
              only available on this device and browser, and could be lost if
              you clear your browser data.
            </p>
            <p>
              Migrate your data to secure cloud storage to access it from any
        ...
        {status === "success" && (
          <Alert className="bg-green-50 text-green-800 dark:bg-green-900/30 dark:text-green-400">
            ...
            <AlertDescription>
              <p className="mb-2">
                Your data has been successfully migrated to secure cloud
                storage.
              </p>
