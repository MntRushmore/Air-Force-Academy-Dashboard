...
export default function TimelinePage() {
  ...
  async function handleUpdateEvent(event: TimelineEvent) {
    try {
      ...
      setEvents(
        events.map((e) =>
          e.id === event.id ? { ...event, isEditing: false } : e
        )
      );
    ...
    <div className="container mx-auto py-6 space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Track your application milestones and important dates
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Add important dates, deadlines, and milestones to your timeline
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="relative pl-8 border-l">
              {filteredEvents.map((event) => (
                <div key={event.id} className="mb-8 relative">
                  <div
                    className={`absolute -left-10 w-5 h-5 rounded-full ${getCategoryColor(
                      event.category
                    )}`}
                  ></div>
                  ...
                  {event.isEditing ? (
                    <div className="bg-muted p-4 rounded-lg space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          <Label htmlFor={`event-category-${event.id}`}>
                            Category
                          </Label>
                          <Select
                            value={event.category}
                            onValueChange={(value) =>
                              handleEventChange(event.id, "category", value)
                            }
                          >
                      ...
                      <div>
                        <Label htmlFor={`event-status-${event.id}`}>
                          Status
                        </Label>
                        <Select
                          value={event.status}
                          onValueChange={(value) =>
                            handleEventChange(event.id, "status", value)
                          }
                        >
